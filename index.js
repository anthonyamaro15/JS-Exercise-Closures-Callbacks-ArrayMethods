 ...
 * should return "sad".
 */
function processContains(item, lists, cb) {
  /* CODE HERE */
  const filtered = lists.includes(item);
  return cb(filtered);
}
...
function processDuplicateFree(list, cb) {
  ...
  const filtered = list.filter((item, i) => list.indexOf(item) === i);
  return cb(filtered);
}
...
function getFullNames(runners) {
  ...
  runners.forEach(runner => {
    let getNames = `${runner.last_name}, ${runner.first_name}`;
    fullName.push(getNames);
  });
  return fullName;
}
...
function getRunnersByTShirtSize(runners, shirtSize) {
  /* CODE HERE */
  const filtered = runners.filter(runner => runner.shirt_size === shirtSize);
  return filtered;
}
